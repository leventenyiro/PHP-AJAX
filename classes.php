            ...
            <select class="form-control" id="input_list_keszleten" onchange="updateKeszleten(<?php echo $this->row['id']; ?>)"> <?php
              if ($this->row["keszleten"] == 1) { ?>
                ...
                <option value="0" selected>Nem</option> <?php
					...
					<td><button class="btn btn-danger" onclick="torles(<?php echo $this->row['id']; ?>)">Törlés</button></td>
          <td><button class="btn btn-primary" onclick=updateForm(<?php echo $this->row["id"]; ?>)>Módosítás</button></td></tr> <?php
			}
    } else { ?>
